...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: Davizin\n' // Nama kamu
            + 'ORG:Davi;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5521999517686:+5521999517686\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\n\nEspero que gostem do grupo seus corno ❤️`
				...
				teks = `Tchau @${num.split('@')[0]} Ja foi tarde cornão 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
